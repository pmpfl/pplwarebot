...
from bot.pplware import Pplware
...
def build_parser():
    parser = argparse.ArgumentParser(description='Run indieshufflebot')

    parser.add_argument('--polling', '-p', dest='polling', type=int, default=2,
                        help='interval (in seconds) to check for message updates')
    parser.add_argument('--db_url', '-d', dest='db_url', default='sqlite:///indiesh.sqlite3',
                        help='URL for database (default is sqlite:///indiesh.sqlite3)')
